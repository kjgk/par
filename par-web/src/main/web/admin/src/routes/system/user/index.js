...
const Component = ({
  ...
  const accountModalProps = {
    ...
    maskClosable: false,
    modalLoading: loading.effects[`${namespace}/showAccountModal`],
    confirmLoading: loading.effects[`${namespace}/saveAccount`],
    title: `设置帐号`,
    onOk (data) {
      dispatch({
        type: `${namespace}/saveAccount`,
        payload: data,
      })
        .then(() => handleRefresh())
    },
    onCancel () {
      dispatch({
        type: `${namespace}/hideAccountModal`,
      })
    },
  }
  ...
  const listProps = {
    ...
    onEditAccount (item) {
      dispatch({
        type: `${namespace}/showAccountModal`,
        payload: item,
      })
    },
    rowSelection: {
  ...
  return (
    <Page inner>
      ...
      {accountModalVisible && <AccountModal {...accountModalProps} />}
    </Page>
  )
}
